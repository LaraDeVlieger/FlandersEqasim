...
"Elle: removed filter, like they wanted to do themselves, because we don't have regions"
...
def execute(context):
    ...
    print(
        "Removing %d/%d (%.2f%%) households (with %d/%d persons, %.2f%%) because at least one person is working outside of Belgium" % (
    ...
    wanted_zones = ["BE2", "BE1"]
    df = df[df["CD_GEO_LVL_1"].isin(wanted_zones)]
    del df["CD_GEO_LVL_1"]
