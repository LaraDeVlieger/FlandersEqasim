...
def sample_destination_municipalities(context, arguments):
    ...
    # Sample destinations
    df_od["count"] = random.multinomial(count, normalized_weights)
...
def sample_locations(context, arguments):
    ...
    if "weight" in df_locations:
        ...
        if np.isnan(weight.any()) or weight.any() == 1:
            weight = [0.00000001 for i in range(0, len(df_locations["weight"].values))]          
