...
def to_gpd(context, df, x = "x", y = "y", crs = "EPSG:3812", column = "geometry"):
    df[column] = [
        geo.Point(*coord) for coord in context.progress(
            zip(df[x], df[y]), total = len(df),
            label = "Converting coordinates"
        )]
    df = gpd.GeoDataFrame(df, crs = "EPSG:3812", geometry = column)

    if not df.crs == "EPSG:3812":
        df = df.to_crs("EPSG:3812")
